...
int Storage_Add_SectorNode(DataType dType, uint8_t sType, dhara_sector_t sector, int32_t sequence);
 ...
 *
 *
 * This is the main way to interact with the NAND storage, without using this the data will likely be
 * deleted on restart during the Storage_Init. The storage initialization insures that the header format
 * is followed and used otherwise the Dhara sector is wiped.
 *
 *
 *     ####      #       #     #   # ## #   #     #       #      ####
 *
 *
 * dType: This is the DataType, this is used to distinguish what type that you are
 * 		attempting to interact with.
 *
 * sType: This is the SectorType, the sector type is used to distinguish what sector batch
 * 		type you are attempting to interact with. This contains the Active batch at index 0 which is what is
 * 		currently being written to and appended to, otherwise it is a backup batch correlating with the number
 * 		used for sType, ie. sType=12 means backup number 12.
 *
 * sequence: This is the sequence number in the sector type batch link list. This is used to order the sectors
 * 		on the NAND flash.
 *
 * SectorNode: This is the node used in the sector type batch link lists, it functions like a normal link list node.
 *
 * SectorHeader: This is used to tag the used Dhara sector so that they can be reloaded if the program is ever restarted.
 *
 *
 *     ####      #       #     #   # ## #   #     #       #      ####
 *
 *
 * Storage_Write is used to overwrite a SectorNode sector that is within the active batch link list.
 *
 * Storage_Append is used to append a SectorNode sector that is within the active batch link list.
 *
 * Storage_Send_To_Backup is used to send an active batch to a backup branch.
 *
 * Storage_Get_SectorNode is used to fetch the SectorNode which then can be read in the Storage_Read function (recommended)
 * 		or use the Dhara_Read wrapper function.
 *
 * Storage_Read is used to read a SectorNode
 *
 * Storage_Delete_Sector is used to remove a SectorNode from the link list
 *
 * Storage_Delete_Sector_List deletes a batch link list
 *
 * Storage_Erase erases the data on a given SectorNode
 *
 * ######################################################################
*/
...
/*
 * RAW type is just for writing anything really, no real requirements but it does not store any backups.
 *
 * TELEM type is used for telemetry data
 *
 * LOG type is used for any logs that need to be stored
 *
 * FIRMWARE is used to store 1 version of any firmware updates.
*/
...
int Storage_Add_SectorNode(DataType dType, uint8_t sType, dhara_sector_t sector, int32_t sequence){
	...
	// If List Is Empty
	if(cur==NULL){
		SECTOR_LIST_HEAD[dType][sType]=newNode;
		return 0;
	}
